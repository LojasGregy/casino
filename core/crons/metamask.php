...
?>
$DataBase->Query("SELECT * FROM 'crypto_transctions' WHERE type = 'metamask' AND status =0");
$DataBase->Execute();
...
foreach($DataBase->ResultSet() as $ms => $row){
    ...
    if($transaction == True) {
        $DataBase->Query("UPDATE 'crypto_transactions' SET Sstatus = 1 WHERE txid = :txid");
        $DataBase->Bind(":txid",$txid);
        $DataBase->Execute();

        $DataBase->Query("UPDATE users SET balance + : amount WHERE userid = userid");
